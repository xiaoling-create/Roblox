...
ButtonContainer.Size = UDim2.new(0, 280, 0, 160)  -- 宽度放大40%，高度放大45%
ButtonContainer.Position = UDim2.new(0.95, -280, 0.5, -80)  -- 右侧定位适配放大后尺寸
ButtonContainer.BackgroundTransparency = 0.8
ButtonContainer.BackgroundColor3 = Color3.new(0, 0, 0)
ButtonContainer.BorderSizePixel = 3  -- 边框加粗
...
TitleLabel.Size = UDim2.new(1, 0, 0, 30)  -- 高度增加50%
TitleLabel.BackgroundTransparency = 1
TitleLabel.Text = "物品过滤"
TitleLabel.TextColor3 = Color3.new(1, 1, 1)
TitleLabel.Font = Enum.Font.GothamBold
TitleLabel.TextSize = 18  -- 文字放大30%
TitleLabel.Parent = ButtonContainer

-- 红卡按钮（放大）
local RedCardButton = Instance.new("TextButton")
RedCardButton.Size = UDim2.new(1, -20, 0, 35)  -- 高度增加40%
RedCardButton.Position = UDim2.new(0, 10, 0, 35)  -- 调整位置适配放大尺寸
...
RedCardButton.TextSize = 16  -- 文字放大15%
...
-- 修复114行报错：定义performServerHop函数（原代码未提前声明导致报错）
local function performServerHop()
    ...
    if targetServer then
        ...
        pcall(function()
            playerData:SetAsync(LocalPlayer.UserId .. "_filters", itemFilters)
        end)
...
local MainScript = [[
    ...
    local savedFilters = pcall(function()
        return playerData:GetAsync(LocalPlayer.UserId .. "_filters")
    end)
    if savedFilters then
        itemFilters = savedFilters
    end
